...
void ProcessFFT::drawAvgGraphUnScaled(ofPoint pt, vector<float> values, ofColor _color){
    ...
    float prevAvgMaximum = 0;
        ...
        for (int i = 0; i < (int)ofMap(values.size(), 0 , values.size(), 0,200); i++){
            ...
            ofVertex(i,ofMap(values[(int)ofMap(i, 0 , 200, 0,values.size())], 0, 80, 200, 0,true));
   ...
   // cout<< "Delta: " << avgMaximum - prevAvgMaximum <<endl;
    ...
    ofSetColor(255);
    ofLine(0,ofMap(avgMaximum, 0, 80, 200, 0,true) , 200, ofMap(avgMaximum, 0,80, 200, 0,true));
...
float ProcessFFT::getIntensityAtFrequency(float _freq){
   ...
   // fft.getBins()[fft.getBins().size()];
    ...
    int whichBin;
    whichBin = ofMap(_freq, 0, 22100, 0, fft.getBins().size()); //
    
    float normalizedFreq;
    normalizedFreq = ofMap(fft.getBins()[whichBin]*scaleFactor, 0, avgMaxSoundOverTime, 0, 1,true); //the 10000 is just a scaling factor to have easier numbers to look at
